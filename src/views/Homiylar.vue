<template >
  <div class=" px-[40px]">
    <div class="flex gap-[12px] justify-center items-center" >
      <table class="tablee ">
        <tr class="  mb-[12px]" >
          ...
          <th class="text-[12px]  text-[#B1B1B8] uppercase font-medium">Tel.Raqami</th>
          <th class="text-[12px]  text-[#B1B1B8] uppercase font-medium">Homiylik summasi</th>
          <th class="text-[12px]  text-[#B1B1B8] uppercase font-medium">Sarflangan summa</th>
          <th class="text-[12px]  text-[#B1B1B8] uppercase font-medium">Sana</th>
          <th class="text-[12px]  text-[#B1B1B8] uppercase font-medium">Holati</th>
    ...
    <tr v-for="(item, i) in data" :key="i"  class="border-[1px] border-[#2E5BFF14] cursor-pointer  ">
          ...
          <td class="text-[15px] text-[#1D1D1F]">{{item.number}}</td>
          <td class="text-[15px]  text-[#1D1D1F]">{{item.summs}} <span class="text-[#B1B1B8]">UZS</span></td>
          <td class="text-[15px]  text-[#1D1D1F]">{{item.summs1}} <span class="text-[#B1B1B8]">UZS</span></td>
          <td class="text-[15px]  text-[#1D1D1F]">{{item.date}}</td>
          <td class="text-[15px]  text-[#1D1D1F]">{{item.position}}</td>
       <router-link to="/edit-student"><td><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
